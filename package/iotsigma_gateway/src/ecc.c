...
*/
static uint8_t curve_p[NUM_ECC_DIGITS] = CONCAT(Curve_P_, 16);
static uint8_t curve_b[NUM_ECC_DIGITS] = CONCAT(Curve_B_, 16);
static EccPoint curve_G = CONCAT(Curve_G_, 16);
static uint8_t curve_n[NUM_ECC_DIGITS] = CONCAT(Curve_N_, 16);
...
static uint8_t vli_testBit(uint8_t *p_vli, uint32_t p_bit)
{
    return (p_vli[p_bit/8] & (1 << (p_bit % 8)));
}

/* Counts the number of 8-bit "digits" in p_vli. */
static uint32_t vli_numDigits(uint8_t *p_vli)
{
    int i;
...
static uint32_t vli_numBits(uint8_t *p_vli)
{
    uint32_t i;
    uint8_t l_digit;
...
static uint8_t vli_lshift(uint8_t *p_result, uint8_t *p_in, uint32_t p_shift)
{
    uint8_t l_carry = 0;
    uint32_t i;
    for(i = 0; i < NUM_ECC_DIGITS; ++i)
...
int ecdsa_verify(EccPoint *p_publicKey, uint8_t p_hash[NUM_ECC_DIGITS], uint8_t r[NUM_ECC_DIGITS], uint8_t s[NUM_ECC_DIGITS])
{
    int i;
    EccPoint l_sum;
    uint8_t *stack = (uint8_t *)buddha_heap_talloc(NUM_ECC_DIGITS * 8, 0);
    uint8_t *u1 = stack;
    uint8_t *u2 = u1 + NUM_ECC_DIGITS;
    uint8_t *z = u2 + NUM_ECC_DIGITS;
    uint8_t *rx = z + NUM_ECC_DIGITS;
    uint8_t *ry = rx + NUM_ECC_DIGITS;
    uint8_t *tx = ry + NUM_ECC_DIGITS;
    uint8_t *ty = tx + NUM_ECC_DIGITS;
    uint8_t *tz = ty + NUM_ECC_DIGITS;
    if (!stack)
        return 0;
    /*
    ...
    */
    ...
    /* Use Shamir's trick to calculate u1*G + u2*Q */
    {
        ...
        uint32_t l_numBits = max(vli_numBits(u1), vli_numBits(u2));
